...
import { addDataToDB, getAllDataFromDB, deleteAllData, getDataWithinDateRange } from "../../util/db";
    ...
    const handleAddData = async () => {
        ...
        for(var plasticType in plasticData.plasticItems) {
            ...
            const newData = {
                ...
                approximateCount: plasticData.plasticItems[plasticType].approximateCount
            };
            ...
            <div className="section-with-space">
                <div className="section-marinelife">
                    <Container fluid>
                        ...
                        <Row>
                            <Col>
                                <Button color="dark" onClick={handleAddData}>Add Data</Button>
                            </Col>
                            <Col>
                                <Button color="dark" onClick={handleDeleteData}>Delete All Data</Button>
                            </Col>
                            <Col>
                                <Button color="dark" onClick={handleSelectedData}>Get Selected Data</Button>
                            </Col>
                        </Row>
                        <Row>
                            <h2>Stored Data:</h2>
                            <ul>
                                {data.map((item, index) => (
                                    <li key={index}>{new Date(item.date).toString()}-{item.date}-{item.type}-{item.weight}</li>
                                ))}
                            </ul>
                        </Row>
                        <Row>
